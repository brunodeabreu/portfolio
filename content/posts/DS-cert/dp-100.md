...
Este é o path sugerido pela Micrsoft para obter o título de **_Microsfot Certified: Azure Sta Scientist Associate_**

![path](/posts/DS-cert/img/ds_100_path.png)

Como podemos ver [Exam AZ-900: Microsoft Azure Fundamentals](https://docs.microsoft.com/en-us/learn/certifications/exams/az-900) não um pré-req para o [Exam DP-100: Designing and Implementing a Data Science Solution on Azure](https://docs.microsoft.com/en-us/learn/certifications/exams/dp-100) e após esse passo o caminho natural é [AI-100 Microsoft Certified: Azure AI Engineer Associate](https://docs.microsoft.com/en-us/learn/certifications/azure-ai-engineer).

Um path de skills é listado abaixo

![skills-DP-100](/posts/DS-cert/img/path_ds_100_2.png)

Em resumo é preciso saber todo pipeline de um ciêntista de dados como descrito na imagem, ou path detalhado de estudo esta no link [Learning path for Azure Data Scientist](https://query.prod.cms.rt.microsoft.com/cms/api/am/binary/RE2PLKZ)


#### O que é Azure Machine Learning ? 

Azure Machine lerning é um conjunto de servições de Cloud turbinado com Python(SDK/CLI/UX) que possibilita:
...
Azure ML inclui o **Worspace** onde você pode criar todo ambiente de _ML Service_, criando modelos, experimentos e pipeline, com data stores e compute targets específicos.

#### 8 passos para treinar um modelo em Azure ML

1. Configurar um workspace
2. Definer os Computes _Clusters : one for Dev and another for Prod_
...
Podemos utilizar RBAC _Role-Based Access Control_ para workspace
...
- Precisa garantir que a **Region** tem os recursos que precisa, nem todas tem GPU por exemplo


- Workspace esta disponivel em duas **Edições**
...
A classe da workspace é o ponto de partida da maioria das operações como no exemplo abaixo que é utilizado `compute_targets` para obter dicionário de objetos contendo compute targers:
```
...
##### Para rodar um Notebook na Workspace
...
> Podemos criar Compute Instances no workspace e termos um ambiente de developement que é gerenciado como todos os assets

##### Porque usar compute instance ? 
...
- Compute target é um recource designado para realizar o treinamento ou servir de host para o serviço de deployment
...
- Azure ML Extention para trabalhar com assets na Azure ML workspace

##### Acessando Dados no Storage

Azure oferece uma conjunto de diversos Datastores listados na figura abaixo, esses são storages para armazenar dados para projetos de data science, projetos, etc.
...
- _TabularDataset_ : dados em formato tabular, facil manipulação com pandas ou spark 

- _FileDataset_ : Representa um único ou vários arquivos no datastore ou URL, com esse método será preciso fazer download, montar e escolher o compute target como FileDataset para consumir o dado.
...
* Um experimento em Azure Machine Learning é um processo, normalmente realizado com scripts ou pipelines que podem gerar metrics e outputs, um experimento pode ser executado diversas vezes e com diferentes dados, códigos e configurações.

* Azure ML monitora cada experimento e oferece a possibilidade de comprar históricos dos resultados.
